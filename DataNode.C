...
void DataNode::Branch(int i, struct Branch b)
{
  branch[i]=b;
}
...
void DataNode::DisconnectBranch(int i)
{
  ...
  // do compaction
  memmove((void *)(&branch[i]), (void *)(&branch[i+1]), (count-i-1)*sizeof(struct Branch));
  count--;
  BranchInit(count);
}
...
{  
  ...
  for (i=0; i<count; i++)
    {
      printf("branch %d\n", i);
//      printf("%d ", (int)(*(branch[i].child)); // TODO
...
int DataNode::AddChild(struct Branch *b, DataNode** new_node, struct split** newsplit)
{
  assert(b);
...
struct split DataNode::MaxExtentBisectionSplit()
{
  ...
  qsort(spreads, NUMDIMS, sizeof(struct dimspread), (int (*) (const void *, const void *)) splitcmp);
  ...
  for(k=0; k<NUMDIMS; k++)
    {
      ...
      splitposition=(coverRect.High(bestSplit.splitDim)+coverRect.Low(bestSplit.splitDim))/2;
  
      for(j=0; j<NODECARD; j++) projection[j]= branch[j].point.At(bestSplit.splitDim);
      // sort it
      qsort(projection, NODECARD, sizeof(float), (int (*) (const void *, const void *)) floatcmp);

      leftcount=0;
      for(j=0; j<NODECARD; j++)
	{
	  if (projection[j] >= splitposition) break;
	  leftcount++;
	}
      // keep the tentative splitposition if it satisfies node utilization guarantees
      // otherwise replace it by the one closest it that does
      minfill=(int)(MINDATANODEUTIL*NODECARD);
      if (minfill < 1) minfill=1;     // minfill should be at least one
      maxfill=NODECARD-minfill;
      if (leftcount < minfill) splitposition=(projection[minfill-1]+projection[minfill])/2;
      else if (leftcount > maxfill) splitposition=(projection[maxfill-1]+projection[maxfill])/2;
...
#ifdef HTREE_DEBUG
  // printf("DataNode split (***** Overlap Free *****): split dimension = %d split position = %f \n", bestSplit.splitDim, bestSplit.splitPos);
#endif HTREE_DEBUG

}

int DataNode::ComputeNumReinsertions(unsigned int splitdimension, float splitposition, int *num_reinsertions)
{
  register int i;
  struct Branch b;
  ...
  for(i=0; i<count; i++)
    {
      assert(branch[i].child);
      if (branch[i].point.At(splitdimension) < splitposition) leftcount++;
      else rightcount++;
    }
...
int DataNode::ComputeReinsertions(unsigned int splitdimension, float splitposition, ReinsertItem *reinsertion_list, int *num_points_in_list)
{
  int leftcount=0, rightcount=0;
  register int i;
  DataNode *replacementNode;
  ...
  if (leftcount < minfill) 
    {
      if (rightcount < minfill) assert(FALSE); // should not happen
      else
	{
	  if (leftcount > 0)
	    {
...
void DataNode::DistributeDataItems(Node *NN, unsigned int splitdimension, float splitposition)
{
  DataNode *replacementNode;
...
int DataNode::IsEmptyPartition(unsigned int splitdimension, float splitposition, int option)
{
  register int i;
  int empty=1;
...
Rect DataNode::CreateELS(unsigned int *ELSarray, Rect *totalspace)
{
  int i;
  Rect livespace;
  if (count > 0) livespace=Cover();
  else
    {
     // this is to ensure that livespace is totally contained within totalspace, otherwise problems
     for(i=0; i<NUMDIMS; i++)
        {
