...
int HTree::Insert(Point *R, int Tid)
{
       ...
       if (num_reinsertions > 0)
	 {
	   ...
	   for (int i=0; i<num_reinsertions; i++)
	     HTreeInsert(&(reinsertion_list[i].point), (int)(reinsertion_list[i].tid), &HTreeRoot, 0);
	 }
...
//
int HTree::HTreeInsert(Point *R, int Tid, Node **Root, int Level, int cascading_splits_allowed=0, ReinsertItem *reinsertList=NULL, int *numItems=NULL)
{
	register Point *r = R;
